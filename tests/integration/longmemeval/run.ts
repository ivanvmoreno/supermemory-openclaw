...
async function main(): Promise<void> {
  ...
  for (const entry of entries) {
    const imported_memories = await importBenchmarkEntry(options, entry, profileRoot);
...
async function resolveOptions(cli: CliOptions): Promise<ResolvedOptions> {
  ...
  const sourceConfig = await maybeReadJson<SourceConfig>(join(sourceStateDir, "openclaw.json"));
  const sourcePluginConfig = sourceConfig?.plugins?.entries?.["openclaw-memory-supermemory"]?.config ?? {};
  const dataFile = resolveDataFile(cli.dataFile);
  const outputDir = join(RUN_ROOT, new Date().toISOString().replace(/[:.]/g, "-"));
  const model = sourceConfig?.agents?.defaults?.model?.primary ?? FALLBACK_QA_MODEL;
...
async function loadEntries(options: ResolvedOptions): Promise<LongMemEvalEntry[]> {
  const data = await readJsonFile<LongMemEvalEntry[]>(options.dataFile);
  let filtered = options.preset === "full" ? [...data] : buildSmokeSelection(data);
...
async function prepareEvalProfile(
  ...
  await maybeCopy(
    join(options.sourceStateDir, "agents", "main", "agent", "auth-profiles.json"),
...
function buildImportMemories(entry: LongMemEvalEntry): ImportMemory[] {
  ...
  for (let sessionIndex = 0; sessionIndex < entry.haystack_sessions.length; sessionIndex++) {
    ...
    for (let roundIndex = 0; roundIndex < rounds.length; roundIndex++) {
      memories.push({
        text: formatSessionBlock("Round Content", sessionDate, rounds[roundIndex]),
...
function toRounds(turns: Array<{ role: string; content: string }>): Array<
...
async function askOpenClaw(
  ...
  const args = ["--profile", options.profile, "--log-level", "error", "agent", "--local"];
  args.push(
    "--json",
    "--session-id",
...
async function runOfficialEval(
  ...
  const evaluatorPath = join(officialRepo, "src", "evaluation", "evaluate_qa.py");
  ...
  const { stdout, stderr } = await runCommand(
    "python3",
    [evaluatorPath, FALLBACK_OFFICIAL_METRIC_MODEL, predictionsPath, options.dataFile],
    {
      cwd: officialRepo,
    },
  );
...
async function readOpenAiApiKey(sourceStateDir: string): Promise<string | undefined> {
  const authPath = join(sourceStateDir, "agents", "main", "agent", "auth-profiles.json");
  if (!existsSync(authPath)) return undefined;

  const auth = await readJsonFile<AuthProfiles>(authPath);
  const profiles = auth.profiles ?? {};
  for (const profile of Object.values(profiles)) {
    if (profile.provider === "openai" && profile.type === "api_key" && profile.key) {
      return profile.key;
    }
  }
  return undefined;
