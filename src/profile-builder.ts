...
export function buildUserProfile(
	...
	const allActive = prioritizePinned(
		db.listActiveMemories(cfg.profileScanLimit),
	)
	...
	for (const memory of allActive) {
		...
		const displayText = sanitizeMemoryTextForPrompt(
			memory.text,
	...
	log?.debug?.(
		`profile built (lt=${longTerm.length}, recent=${recent.length}) from ${allActive.length} active memories`,
	)
...
export function getCachedProfile(
	...
	const longTerm = dedupeMemoryTexts(
		JSON.parse(longTermCache.content) as string[],
	)
...
export function shouldRebuildProfile(
	...
	if (interactionCount > 0 && interactionCount % cfg.profileFrequency === 0) {
		log?.debug?.(
			`profile rebuild triggered (frequency hit at interaction=${interactionCount})`,
		)
		return true
	}
	...
	if (ageMs > PROFILE_REBUILD_MAX_AGE_MS) {
		log?.debug?.(
			`profile rebuild triggered (stale, age=${Math.round(ageMs / 1000)}s)`,
		)
		return true
	}

	log?.debug?.(
		`profile cache hit (age=${Math.round(ageMs / 1000)}s, lt=${cached.longTerm.length}, recent=${cached.recent.length})`,
	)
	return false
}
