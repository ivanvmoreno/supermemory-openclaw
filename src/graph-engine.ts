...
const DEDUP_STOPWORDS = new Set<string>(Object.values(stopwordsIso).flat())
...
export async function processNewMemory(
	...
	const memoryType =
		options?.memoryTypeOverride ?? options?.semanticMemory?.memoryType ?? "fact"
	const pinned = options?.pinnedOverride ?? false
	const expiresAt =
		memoryType === "episode"
			? parseExpiresAtIso(options?.semanticMemory?.expiresAtIso)
			: null
	...
	if (exactDuplicate) {
		log?.debug?.(
			`exact duplicate → merging into ${exactDuplicate.id.slice(0, 8)}`,
		)
		return (
	...
	if (lexicalDuplicate) {
		log?.debug?.(
			`lexical duplicate → merging into ${lexicalDuplicate.id.slice(0, 8)}`,
		)
		return (
			db.mergeDuplicateMemory({
	...
	if (options?.embeddingEnabled !== false) {
		try {
			...
			if (nearDuplicate) {
				log?.debug?.(
					`near-duplicate (vector) → bumping access on ${nearDuplicate.id.slice(0, 8)}`,
				)
				...
				for (const mention of nearDupMentions) {
					...
					const resolved = db.resolveEntityAlias(
						mention.mention,
						normalized,
						mention.kind,
					)
	...
	if (parentMemoryId) {
		log?.debug?.(
			`update relationship → supersedes ${parentMemoryId.slice(0, 8)}`,
		)
		try {
	...
	addDeterministicRelatedEdges(
		memory,
		db,
		parentMemoryId ? new Set([parentMemoryId]) : new Set(),
		options?.cfg,
		log,
	)
	return memory
...
function findNearDuplicate(
	...
	const hydrated = db.getMemoriesByIds(
...
function findLexicalDuplicate(
	...
	const candidates = db.getMemoriesByIds(
		db
			.ftsSearch(queryTerms.join(" "), DEDUP_FTS_CANDIDATE_LIMIT)
...
function lexicalDuplicateScore(left: string, right: string): number {
	...
	const contentContainment = setContainmentScore(
		new Set(leftContent),
		new Set(rightContent),
	)
	const tokenLcsScore =
...
function normalizeForDedup(text: string): string {
	return text.normalize("NFKD").replace(DIACRITIC_PATTERN, "").toLowerCase()
}
...
function isContentToken(token: string): boolean {
	return (
		isStructuredToken(token) ||
		(token.length >= 3 && !DEDUP_STOPWORDS.has(token))
	)
}
...
async function resolveUpdateRelationship(
	...
	const effectiveLog: SemanticLogLike = log ?? {
		info: () => {},
		warn: () => {},
	}
	...
	const updateDecision = decisions.find(
		(decision) => decision.relationType === "updates",
	)
...
function buildUpdateCandidates(
	...
	function upsertCandidate(
		candidate: MemoryRow,
		options?: { sharedEntity?: boolean; vectorScore?: number },
	) {
...
function addDeterministicRelatedEdges(
	...
	const candidateMap = new Map<
		string,
		{ memory: MemoryRow; sharedEntityCount: number }
	>()
	...
	if (edgesAdded > 0) {
		log?.debug?.(
			`added ${edgesAdded} related edges for ${memory.id.slice(0, 8)}`,
		)
	}
}
...
export function buildEntityMergeCandidates(
	...
	for (let i = 0; i < entities.length; i++) {
		for (let j = i + 1; j < entities.length; j++) {
			...
			const stringScore = stringSimilarity(
				left.canonicalName,
				right.canonicalName,
			)
...
function stringSimilarity(left: string, right: string): number {
	const leftTokens = new Set(
		normalizeEntityAliasText(left).split(/\s+/).filter(Boolean),
	)
	const rightTokens = new Set(
		normalizeEntityAliasText(right).split(/\s+/).filter(Boolean),
	)
	...
	const foldedEqual =
		foldForLooseComparison(left) === foldForLooseComparison(right) ? 1 : 0
...
function cosineSimilarity(
	left: Float64Array | null,
	right: Float64Array | null,
): number {
...
export function formatEntityMergeInputs(
	...
	return db.listCanonicalEntities(limit).map((entity) => ({
		...
		aliases: db
			.getAliasesForEntity(entity.id)
			.map((alias) => alias.surface_text),
	}))
}

export function getEntityIdsForMemoryMentions(
