...
class OllamaEmbeddingProvider implements EmbeddingProvider {
	...
	async embedBatch(texts: string[]): Promise<Float64Array[]> {
		...
		if (!response.ok) {
			const errText = await response.text().catch(() => "unknown error")
			throw new Error(
				`Ollama embed batch failed (${response.status}): ${errText}`,
			)
		}
		...
		if (!data.embeddings || data.embeddings.length !== texts.length) {
			throw new Error(
				`Ollama returned ${data.embeddings?.length ?? 0} embeddings for ${texts.length} texts`,
			)
		}
...
export function createEmbeddingProvider(
	...
	if (!config.enabled) {
		return {
			...
			async embed() {
				return new Float64Array(vectorDims)
			},
			async embedBatch(texts) {
				return texts.map(() => new Float64Array(vectorDims))
			},
		}
	}
	...
	if (isOllamaProvider(config.provider)) {
		...
		if (!config.apiKey) {
			throw new Error(
				`API key required for embedding provider "${config.provider}"`,
			)
		}
