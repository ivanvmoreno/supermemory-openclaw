...
import { MEMORY_TYPES, type MemoryType, type SupermemoryConfig } from "./config.ts";
...
export function createMemorySearchTool(ctx: ToolContext): ToolDefinition {
  return {
    ...
    parameters: Type.Object({
      ...
      limit: Type.Optional(
        Type.Number({ description: "Max results (default: configured maxRecallResults)" }),
      ),
    }),
  ...
  return {
    ...
    parameters: Type.Object({
      ...
      pinned: Type.Optional(
        Type.Boolean({ description: "Pin this memory so it never decays automatically" }),
      ),
    }),
    async execute(_toolCallId, params) {
        ...
        content: [
          {
            type: "text",
            text:
              (storedMemories.length === 1 ? "Stored 1 memory:\n" : `Stored ${storedMemories.length} memories:\n`) +
              summaryLines.join("\n"),
          },
        ],
        details: {
  ...
  return {
    ...
    async execute(_toolCallId, params) {
      ...
      if (query) {
        ...
        if (results.length === 1 && results[0].score > FORGET_AUTO_DELETE_MIN_SCORE) {
        ...
        const list = results
          .map((result) => `- [${result.memory.id.slice(0, 8)}] ${result.memory.text.slice(0, 60)}...`)
          .join("\n");
    ...
    async execute(_toolCallId, params) {
      ...
      const longTermText =
        profile.longTerm.length > 0
          ? profile.longTerm.map((item, index) => `${index + 1}. ${item}`).join("\n")
          : "(no long-term memories yet)";

      const recentText =
        profile.recent.length > 0
          ? profile.recent.map((item, index) => `${index + 1}. ${item}`).join("\n")
          : "(no recent context)";
